...
This writes `pipeline.mmd` and `pipeline.svg` to the same directory as the STAR file.
...
```bash
relion_pipeline_visualizer path/to/default_pipeline.star --no-svg
```
...
options:
  ...
  -o, --output FILE     Output path for .mmd file (default: same directory as star_file)
  --no-svg              Skip SVG generation (SVG is generated by default)
```
...
SVG files are generated automatically by default. You can also paste the `.mmd` output
into https://mermaid.live to visualize interactively.
